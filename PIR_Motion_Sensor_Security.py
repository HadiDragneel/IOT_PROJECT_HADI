...
IntruderID = 0               #Used to identify a motion/intruder as a number for Thingspeak.


def NotifyUser(): #Uses Pushbullet API to send high priority notification to phone. This notification will always make a sound at max volume.
    ...
    while True:                 #Infinite loop to scan until stopped manually by the user.
        ...
        if input == 1:
            ...
            if D == 1:        #This prevents spamming. D goes up with signals from PIR Sensor and notifies user 1 time per movement.
                ...
                UpdateThingspeak()
                time.sleep(5) #This wait function is also meant to help avoid spam.
            time.sleep(1)
